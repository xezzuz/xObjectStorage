...
public class IntegrityRegistry {
	...
	public void add(IntegrityReport report) {
		log.finest("Adding integrity report for segment " + report.getSegmentId() +
	...
	public IntegrityReport get(int segmentId) {
		...
		if (report == null) {
			log.warning("No integrity report found for segment " + segmentId);
		} else {
			log.finest("Retrieved integrity report for segment " + segmentId +
					", health: " + report.getHealthStatus());
		}
		return report;
	}
